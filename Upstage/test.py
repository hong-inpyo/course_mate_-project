...
임베딩 모델: solar-embedding-2-query / solar-embedding-2-passage
    ...
    - 8K 컨텍스트 지원
    - 아직 별도 alias가 없어서 전체 모델명 그대로 사용해야 함
    - Rate limit: 100 RPM / 300,000 TPM (143개 청크 정도는 문제없이 처리됨)

사전 준비:
    pip install chromadb openai requests --break-system-packages
    이 파일과 같은 폴더에 secrets.json 파일을 두고, 안에 아래처럼 키를 저장:
        { "UPSTAGE_API_KEY": "발급받은키" }

사용법:
    python pipeline.py                    # chunks.jsonl 임베딩해서 벡터DB 구축
    python pipeline.py ask "질문 내용"      # 구축된 벡터DB로 질문에 답변
"""
...
SECRETS_FILE = os.path.join(_BASE_DIR, "secrets.json")
...
def load_api_key(path=SECRETS_FILE):
    """같은 폴더의 secrets.json에서 API 키를 읽어온다.
    키 이름은 UPSTAGE_API_KEY / upstage_api_key / api_key 중 아무거나 지원."""
    if not os.path.exists(path):
        raise SystemExit(f"secrets.json을 찾을 수 없습니다: {path}")
    ...
    raise SystemExit(
        f"secrets.json 안에서 API 키를 찾지 못했습니다. "
        f"'UPSTAGE_API_KEY' 같은 키 이름으로 저장되어 있는지 확인해주세요. "
        f"(파일 안 키 목록: {list(secrets.keys())})"
    )
