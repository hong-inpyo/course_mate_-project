...
import eligibility          # 수강자격(수강대상 문구) 해석 — 결정론적, API 미사용
...
def slots_overlap(a, b):
    ...
    if a[0] != b[0]: #요일이 안 겹치면 애초에 겹칠 일이 없음
        return False
    return a[1] < b[2] and b[1] < a[2] # a시작 < b종료 and b시작 < a종료-> a시간표 중간에 b가 위치 또는 b시간표 중간에 a가 위치
...
def _make_sections(g):
    ...
    for _, row in g.iterrows():
        ...
        sections.append({
            ...
            "교수": _col(row, "교수명"),
            "언어": _col(row, "강의언어"),
            "수강대상": _col(row, "수강대상"),        # 누가 들을 수 있는지(자유서술) — eligibility가 해석
            "외국인전용": _col(row, "외국인"),
            "내국인전용": _col(row, "내국인"),
            "개설학과": _col(row, "개설학과"),        # '타전공생 수강불가' 판정용
        })
    return sections
...
def build_candidates(df, dept, 이수구분=("전공필수", "전공기초"), target_years=None,
                     include_cyber=False):
    #build_candidates에는 load_coures를 거친 엑셀이 들어감(원본에 slots, rooms 추가한 엑셀)
    ...
    if target_years is not None:
        sub = sub[pd.to_numeric(sub[ycol], errors="coerce").isin(target_years)] #sub의 학년을 숫자타입으로 변환하고 target_years가 있는 행만 필터링
    courses = []
    for hakbun, g in sub.groupby("학수번호"):
        ...
        courses.append({
            ...
            "학년": r0[ycol],
            "학점": float(r0["학점"]),
            "priority": PRIORITY.get(r0["이수구분"], 9), #9는 탐색 실패시 반환값
            "sections": _make_sections(g),
        })
    # 우선순위(전공필수 먼저) → 학점 큰 것 먼저
    #-를 붙여야 절댓값이 가장 큰 숫자가 가장 왼쪽에 온다
    courses.sort(key=lambda c: (c["priority"], -c["학점"]))

    #courses는 특정 학수번호의 교과목명, 이수구분, 학과전공, 학년, 학점, 이수구분의 우선순위, 분반 행들의 섹션 리스트(시간, 강의실, 교수, 언어, 사이버 여부) 
    return courses
...
def apply_section_filters(courses, profile, enroll_limits=None, known_depts=None):
    ...
    enroll_limits: eligibility.build_limits()가 만든 {수강대상문구: 표준제한}.
      주면 학생이 들을 수 없는 분반(타학과 대상·외국인 전용 등)을 제외한다.
      판정은 결정론적이며 애매하면 통과시킨다(eligibility.is_eligible 참조).
    """
...
def solve(courses, target_credits=18, prefer="오전", max_courses=8, group_limits=None,
          prefs=None, fixed=None, node_budget=None):
    """
    ...
    def dfs(i, chosen, used_sections, credits):
        # node_budget이 있으면 탐색량을 제한한다. best는 노드마다 갱신되므로 끊어도 유효한 시간표가 남는다.
        # (후보가 서로 잘 안 겹치는 학과에서 조합 폭발 → 데모가 멈추는 것을 막기 위한 안전장치)
...
def recommend_for_profile(profile, cur_xlsx, old_xlsx, rename_json,
                          ...
                          df=None, fixed_courses=None, node_budget=None,
                          enroll_limits=None):
    ...
    # 수강자격: '수강대상및유의사항' 문구를 표준 제한으로 정규화(결정론적, 고유값만 → 즉시).
    # 실존 학과·대학명을 넘겨 '수강과목'의 '수강과' 같은 오인을 막는다.
    # enroll_limits를 호출측이 주면 그걸 쓴다 — 문구가 자유서술이라 규칙으로 부족한 학교는
    # 밖에서 만든 표준 제한(예: 이화여대는 Solar 파싱)을 넘긴다.
    known_depts = set(df["개설학과전공"].dropna().astype(str)) if "개설학과전공" in df.columns else None
    if enroll_limits is None and "수강대상및유의사항" in df.columns:
        enroll_limits = eligibility.build_limits(df["수강대상및유의사항"].dropna().unique(),
                                                 eligibility.orgs_from_df(df))
    ...
    def _dedup_solve(pool_):
        ...
        pool_, dropped_ = apply_section_filters(pool_, profile, enroll_limits,
                                               known_depts)   # 하드 조건 먼저
        ...
        chosen_ = solve(cands_, target_credits=target_credits,
                        ...
                        prefs=prefs, fixed=fixed_courses, node_budget=node_budget)
